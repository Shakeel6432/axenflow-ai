  ...
  return (
    <Section divider>
      <HomeReveal>
        <SectionHeading
          title="How We Work"
          description="Start with free tools on AxenFlow, then we build whatever still needs a custom system."
        />
      </HomeReveal>
      <div className="relative">
        <div className="step-line pointer-events-none absolute left-[12%] right-[12%] top-[2.5rem] hidden lg:block" />
        <HomeReveal stagger className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {processSteps.map((step, i) => (
            <div
              key={step.step}
              className="glass-card home-hover-lift relative rounded-xl p-7 text-center lg:text-left"
            >
              <div
                className={`mx-auto mb-5 flex h-12 w-12 items-center justify-center rounded-xl ${stepColors[i].bg} ${stepColors[i].shadow} text-sm font-bold text-white shadow-lg lg:mx-0`}
              >
                {step.step}
              </div>
